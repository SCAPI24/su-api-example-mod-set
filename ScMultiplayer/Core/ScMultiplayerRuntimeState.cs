...
namespace ScMultiplayer
{
    ...
    public partial class ScMultiplayer
    {
        ...
        // 最近一次"主机判死"所用的权威状态序号。客户端本地复活后，主机那份可能还停在 0
        // （复活请求还没被处理）—— 用"这个序号是否晚于复活那一刻记下的序号"来区分
        // "过期的主机判死"（不能跟随，否则刚站起来就被写回 0）与"复活之后新判的死"（必须生效）。
        private int m_localDeathSequence;
        // 本端是否已经把"这次主机判死"落地过（写死因 + 记一条死亡统计）——
        // 只在由活转死的那一次落地，避免每秒广播重复记录。
        private bool m_localDeathApplied;
        // 这次死亡的死因是否已经拿到**真实**死因（不是 Unknown / 也不是本 Mod 自己那几个上报标签）。
        // 主机判死可能先由"受击击退"那条快照送到（它原本不带死因），随后周期快照才带着真死因到；
        // 拿到真死因之前允许再落地一次把死因补上（实测："被狼杀死还是未知原因"）。
        private bool m_localDeathCauseResolved;
