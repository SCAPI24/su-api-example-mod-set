...
所有 Mod 已迁移至 net8.0 双平台（net8.0 + net8.0-android），使用 SDK 样式 csproj。
...
字符串翻译 Mod，通过 Widget 树文本拦截 + IStringProcessor 翻译接口，将游戏界面翻译为中文。支持 .lst 字体加载、动态译文收集、XML 翻译表导出。演示 `LoadingManager.QueueItem` 和 `LoadingManager.ReplaceItem` 用法。
...
- **UdpTransmitter 构造函数变更**：net8.0 版 `UdpTransmitter(int localPort = 0)`，自动检测 LAN 地址
...
14. **移除 Logging.Debug**：模板默认引入 Microsoft.Extensions.Logging.Debug，Android logcat 大量输出导致卡顿
15. **global.json SDK锁定**：项目根目录 global.json 锁定 SDK 8.0.402 + rollForward: disable，必须从项目根目录运行 dotnet 命令
16. **.scmod ZIP 正斜杠铁律**：必须用 Python zipfile 打包，Windows Compress-Archive 生成反斜杠路径→ModLoader `Contains("Lib/Arm64")` 匹配失败→No DLL found→Mod 不加载
17. **ModInfo.xml 嵌套格式**：根元素必须是 `<Mod>`，内嵌 `<ModInfo>` 和 `<Dependencies>`。扁平格式→ModLoader 返回 null→"Invalid ModID"→静默跳过
18. **.scmod ModInfo.xml 根目录**：打包时 ModInfo.xml 必须放在 ZIP 根目录，误入 Content/→ModLoader 找不到→静默跳过
19. **PowerShell `[]` 通配符**：操作含 `[SuAPI]` 路径时必须用 `-LiteralPath`
20. **APK 文件名含[]**：apksigner `--out` 不支持含 `[]` 路径，先签名到临时文件再 `Move-Item -LiteralPath` 重命名
