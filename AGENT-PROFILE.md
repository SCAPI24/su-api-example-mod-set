...
### 19. SubsystemGameWidgets 只能被一个 Mod 替换

ConsoleMod 已替换 SubsystemGameWidgets（GUID 6bf14dc6-32e7-4e8c-b3c4-438e0eee13ad），其他 Mod 不能再替换同一个 Subsystem。需要 UI overlay 的 Mod 应改用 ComponentTemplate+IUpdateable 独立 Component 模式，挂到 Player 实体，每帧 Attach/Detach Widget。参考 WatchMod 实现。

### 20. Component.Load 跨assembly override 签名

`Component.Load(ValuesDictionary, IdToEntityMap)` 是 `protected internal virtual`，跨assembly只能用 `protected override`（不是 `protected internal override`）。IUpdateable.Update 用显式接口实现 `void IUpdateable.Update(float dt)`。DatabaseObject 构造时设 NestingParent 即自动注册，无 AddDatabaseObject 方法。
