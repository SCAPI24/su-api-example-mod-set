...
The project's named remote headless server is `<server-ip>:22`, with the
...
server at `<server-ip>:22`, whose installation root is always
...
```text
py -3 tools/check_remote_runtime.py --host <server-ip> --user <ssh-user>
py -3 tools/deploy_remote_mod.py --host <server-ip> --user <ssh-user> --source <package.scmod>
py -3 tools/deploy_remote_current.py --host <server-ip> --user <ssh-user> --source <windows-publish-directory>
```
