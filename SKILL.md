...
description: Survivalcraft 2 SuMod 开发技能。帮助创建、修改、调试第三方 Mod。触发词：Survivalcraft、SC mod、SuMod、IMod、scmod、游戏mod、冷雨夜、联机mod、TemperatureImmunity、RainWithoutDawn、ScMultiplayer。当用户要求创建/修改/调试 Survivalcraft 的 Mod，或提到 SuAPI/SuMod/IMod 接口，或提到 SurvivalcraftMonoWinAN 目录下的 Mod 开发时触发。技能支持引用本地项目和 Git 仓库，公开仓库：https://gitee.com/SC-SPM/survivalcraft-su-api
---
...
**当前活跃项目(net8.0 Mono版)**:
```
D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN\
├── Engine/              # 引擎层(Engine.dll)
├── EntitySystem/
│   ├── GameEntitySystem/  # 游戏实体系统(含 GameDatabase.cs,AssemblyName=GameEntitySystem)
│   ├── SuMod/            # ★ Mod 核心接口与实现
│   ├── TemplatesDatabase/ # 模板数据库
│   └── XmlUtilities/     # XML 工具库
...
├── Survivalcraft.sln    # Windows 解决方案
├── SurvivalcraftAn.sln  # Android 解决方案
├── global.json          # SDK 8.0.402 锁定
└── publish/             # 发布输出
    ├── win-x64/         # Windows 发布版(self-contained)
    │   ├── Mods/        # ★ Mod 放置目录
    │   └── Logs/Game.log
    └── android/         # Android 发布版(APK)
```

**旧版项目(已淘汰,仅参考)**: `P:\UGIT\Survivalcraft\`(net10.0 MAUI版)
...
├── Survivalcraft.dll      # 游戏逻辑
├── Engine.dll             # 引擎
├── GameEntitySystem.dll   # 实体系统
├── Mods/                  # ★ Mod 放置目录(.scmod 或 .dll)
├── Logs/Game.log          # ★ 运行时日志
└── ...                    # self-contained,含 .NET Runtime
```

**Android 发布版**:
```
publish\android\
└── [SuAPI]Survivalcraft-x.x.x.x.Apk  # ~52.4MB
```

**Mod 编译输出**(以 StringInterceptor 为例):
```
Mod\{ModName}\bin\Debug\
├── net8.0\                    # Windows 编译输出
│   ├── {ModName}.dll          # 未混淆 DLL
│   └── Obfuscar\{ModName}.dll # ★ 混淆后 DLL(打包用这个)
└── net8.0-android\            # Android 编译输出
    └── {ModName}.dll          # 直接输出(无混淆)
```
...
**日志路径**:
- **Windows (net8发布版)**: `D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN\publish\win-x64\Logs\Game.log`
- **Windows (MAUI版, 已淘汰)**: `Survivalcraft\bin\Debug\net10.0-windows10.0.19041.0\win-x64\Logs\Game.log`
- **Android**: `/sdcard/Download/Survivalcraft2/Logs/Game.log`(GameLogSink.cs `#if ANDROID` 分支用 FileStream 直接写外部存储)

**编译发布**:
- ⚠ **global.json SDK锁定**: 项目根目录 global.json 锁定 SDK 8.0.402 + rollForward: disable,**必须从项目根目录运行 dotnet 命令**,否则 SDK 10.0 对 net8.0-android 报 NETSDK1202
- **Windows**: `dotnet publish Survivalcraft\Survivalcraft.csproj -c Release -r win-x64 --self-contained true -o publish\win-x64`
- **Android**: `dotnet publish Survivalcraft\SurvivalcraftAndroid.csproj -c Release -f net8.0-android -o publish\android`
- **APK签名**: zipalign → apksigner sign (Suceru.jks, alias suceru) → 验证 V1/V2/V3
- ⚠ **APK文件名含[]时apksigner --out失败**: 先签名到临时文件,再用 `Move-Item -LiteralPath` 重命名
- ⚠ **PowerShell `[]` 通配符**: 操作含[]路径时必须用 `-LiteralPath`
- **版本号位置**: `SurvivalcraftAndroid.csproj` 的 `<ApplicationDisplayVersion>`
...
**当前(net8.0 Mono版)**:`Loading.Initialize` 事件参数为 `new object[] { typeof(LoadingManager) }`。`LoadingManager` 是静态类,通过 `QueueItem`/`ReplaceItem` 操作加载队列。
...
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    ...
    <CheckEolTargetFrameworks>false</CheckEolTargetFrameworks>
  ...
  <ItemGroup Condition="'$(TargetFramework)' == 'net8.0'">
    ...
    <ProjectReference Include="..\..\EntitySystem\SuMod\SuMod.csproj" />
    <ProjectReference Include="..\..\EntitySystem\TemplatesDatabase\TemplatesDatabase.csproj" />
    <ProjectReference Include="..\..\EntitySystem\XmlUtilities\XmlUtilities.csproj" />
    <ProjectReference Include="..\..\Survivalcraft\Survivalcraft.csproj" />
  </ItemGroup>
  <!-- Android: DLL reference -->
  <ItemGroup Condition="'$(TargetFramework)' == 'net8.0-android'">
    <Reference Include="Engine">
      <HintPath>..\..\Engine\Engine.Android\bin\Debug\net8.0-android\Engine.dll</HintPath>
      <Private>false</Private>
    </Reference>
    <Reference Include="GameEntitySystem">
      <HintPath>..\..\EntitySystem\GameEntitySystem\bin\Debug\net8.0-android\GameEntitySystem.dll</HintPath>
      <Private>false</Private>
    </Reference>
    <Reference Include="SuMod">
      <HintPath>..\..\EntitySystem\SuMod\bin\Debug\net8.0-android\SuMod.dll</HintPath>
      <Private>false</Private>
    </Reference>
    <Reference Include="TemplatesDatabase">
      <HintPath>..\..\EntitySystem\GameEntitySystem\bin\Debug\net8.0-android\TemplatesDatabase.dll</HintPath>
      <Private>false</Private>
    </Reference>
    <Reference Include="XmlUtilities">
      <HintPath>..\..\EntitySystem\GameEntitySystem\bin\Debug\net8.0-android\XmlUtilities.dll</HintPath>
      <Private>false</Private>
    </Reference>
    <Reference Include="Survivalcraft">
      <HintPath>..\..\Survivalcraft\bin.android\net8.0-android\Survivalcraft.dll</HintPath>
      <Private>false</Private>
    </Reference>
  </ItemGroup>
  <ItemGroup>
  ...
  <!-- Obfuscar only for Windows -->
  <Target Name="PostBuild" AfterTargets="PostBuildEvent" Condition="'$(TargetFramework)' == 'net8.0'">
...
**关键点**:
- 双 TFM: `net8.0` + `net8.0-android`
- Windows 用 `ProjectReference`,Android 用 `Reference + HintPath`
- GameEntitySystem.csproj(AssemblyName=GameEntitySystem)
...
- `ANDROID`/`WINDOWS` 编译符号通过 csproj DefineConstants 条件设置
...
<Obfuscator>
    ...
    <Module file="$(InPath)\GameEntitySystem.dll"/>
    <Module file="$(InPath)\MyMod.dll"/>
</Obfuscator>
```

### 双平台打包脚本(Python zipfile,确保正斜杠路径)

> ⚠ **铁律**: Windows Compress-Archive 生成反斜杠路径 `Lib\Arm64\`,ModLoader 用 `Contains("Lib/Arm64")` 匹配正斜杠→失败→Mod 不加载。必须用 Python zipfile 打包。

```powershell
$modName = "MyMod"
$modDir = "D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN\Mod\$modName"
$winDll = "$modDir\bin\Debug\net8.0\Obfuscar\$modName.dll"     # Windows 混淆后
$androidDll = "$modDir\bin\Debug\net8.0-android\$modName.dll" # Android 直接输出
$winMods = "D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN\publish\win-x64\Mods"

# 1. 编译(必须从项目根目录)
Set-Location "D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN"
dotnet build "Mod\$modName\$modName.csproj" -c Debug --verbosity minimal

# 2. 准备打包目录
$packDir = Join-Path $env:TEMP "pack_$modName"
if (Test-Path $packDir) { Remove-Item -Recurse -Force $packDir }
New-Item -ItemType Directory -Path "$packDir\Lib\X64", "$packDir\Lib\Arm64" -Force | Out-Null

Copy-Item $winDll "$packDir\Lib\X64\$modName.dll"
Copy-Item $androidDll "$packDir\Lib\Arm64\$modName.dll"
Copy-Item "$modDir\ModInfo.xml" "$packDir\"           # ★ 根目录,不能放入 Content/
if (Test-Path "$modDir\Content") { Copy-Item -Recurse "$modDir\Content" "$packDir\Content" }

# 3. Python zipfile 打包(确保正斜杠路径)
$scmodName = "[SuAPI]中文名.scmod"  # ★ 必须加 [SuAPI] 前缀
$scmodPath = "$winMods\$scmodName"
...
python3 -c "
import zipfile, os
pack_dir = os.environ.get('TEMP') + '\\pack_$modName'
temp_zip = os.environ.get('TEMP') + '\\$modName.zip'
with zipfile.ZipFile(temp_zip, 'w', zipfile.ZIP_DEFLATED) as zf:
    for root, dirs, files in os.walk(pack_dir):
        for f in files:
            full_path = os.path.join(root, f)
            arc_name = os.path.relpath(full_path, pack_dir).replace(os.sep, '/')
            zf.write(full_path, arc_name)
"

# 确保 Mods 目录存在
if (!(Test-Path $winMods)) { New-Item -ItemType Directory -Path $winMods -Force | Out-Null }
Move-Item -LiteralPath $tempZip $scmodPath    # ★ -LiteralPath 处理 [SuAPI]

# 4. 验证 ZIP 结构
python3 -c "
import zipfile
with zipfile.ZipFile(r'$scmodPath', 'r') as zf:
    for e in zf.infolist():
        print(f'{e.file_size:>10}  {e.filename}')
    names = [e.filename for e in zf.infolist()]
    print(f'ModInfo.xml in root: {"ModInfo.xml" in names}')
    print(f'Lib/Arm64/ exists: {any("Lib/Arm64/" in n for n in names)}')
    print(f'Lib/X64/ exists: {any("Lib/X64/" in n for n in names)}')
    print(f'No backslash paths: {all(chr(92) not in n for n in names)}')
"
```
...
1. 在 `D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN\Mod\` 下创建 Mod 目录
2. 创建 .csproj(SDK 样式,双 TFM: net8.0 + net8.0-android)
3. 创建 `Plug/` 目录放 IMod 入口类
4. 按需创建 `Component/`、`Func/` 等子目录放替换类
5. 实现 IMod 接口,在 OnLoad 中订阅事件或注册 Injector
6. 编写替换类(继承原始类,重写必要方法)
7. 创建 ModInfo.xml(嵌套格式 `<Mod><ModInfo>...</ModInfo></Mod>`)和 Obfuscar.xml
8. 从项目根目录编译 + Python zipfile 打包 .scmod
...
**net8.0 项目**:从项目根目录运行(global.json SDK锁定)

```powershell
# 必须从项目根目录执行
Set-Location "D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN"
dotnet build "Mod\MyMod\MyMod.csproj" -c Debug --verbosity minimal
```
...
- net8.0 编译输出含 Obfuscar 混淆步骤(仅 Windows TFM),最终 DLL 在 `bin\Debug\net8.0\Obfuscar\` 目录。Android DLL 在 `bin\Debug\net8.0-android\` 直接输出
...
- **编译必须用 MSBuild**:仅限 net48 项目。net8.0 项目可用 `dotnet build`(必须从项目根目录运行,global.json SDK锁定)
...
- **LoadingManager.ReplaceItem vs QueueItem**: 替换 Screen 必须用 `ReplaceItem("Initialize XxxScreen", action)`,不能用 `QueueItem` + 同名 `AddScreen`--后者会导致 `ArgumentException: An item with the same key has already been added. Key: Play`。name 参数是 QueueItem 注册名(如 "Initialize PlayScreen"),不是 Screen 名(如 "Play")
...
- **⚠️ .scmod ZIP 路径必须用正斜杠**: Windows Compress-Archive 生成反斜杠路径 `Lib\Arm64\`,ModLoader 用 `Contains("Lib/Arm64")` 匹配正斜杠→失败→`No DLL found`→Mod 不加载。**必须用 Python zipfile 打包**,`arc_name.replace(os.sep, '/')` 确保正斜杠。打包后用 Python 验证: `all(chr(92) not in e.filename for e in zf.infolist())`
...
# 列出所有已安装 Mod(含已禁用)
$winMods = "D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN\publish\win-x64\Mods"
Get-ChildItem -LiteralPath "$winMods\*" -Include *.scmod,*.scmod.unint | ForEach-Object {
...
# 禁用指定 Mod
$winMods = "D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN\publish\win-x64\Mods"
Rename-Item -LiteralPath "$winMods\TargetMod.scmod" "TargetMod.scmod.unint"
```
...
# 启用指定 Mod
$winMods = "D:\Users\Suceru\Desktop\jiejian\SurvivalcraftMonoWinAN\publish\win-x64\Mods"
Rename-Item -LiteralPath "$winMods\TargetMod.scmod.unint" "TargetMod.scmod"
```
...
- 目标: ARM64 真机 (<phone-model>, Adreno 740, Android 15) + x86_64 模拟器
...
```bash
# 安装与启动
adb -s <phone-serial> push <apk> /data/local/tmp/sc2.apk
adb -s <phone-serial> shell pm install -r /data/local/tmp/sc2.apk
adb -s <phone-serial> shell am start -n com.candyrufus.survivalcraft2/crc64138d896b0a338567.MainActivity

# 日志 (logcat 在 Android 15 上不可用)
adb -s <phone-serial> shell "run-as com.candyrufus.survivalcraft2 cat files/Documents/Logs/Game.log"

# Native crash 分析
adb -s <phone-serial> shell dumpsys dropbox data_app_native_crash --print

# 权限
adb -s <phone-serial> shell appops set com.candyrufus.survivalcraft2 MANAGE_EXTERNAL_STORAGE allow
```
...
public static float ApproximateScreenInches => √(W2+H2) / ApproximateScreenDpi;
